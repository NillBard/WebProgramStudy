...
Revision ID: 8fa5da923cd1
Revises: 
Create Date: 2021-12-13 17:51:49.750772
...
def upgrade():
    ...
    op.create_table('imports',
                    ...
                    sa.PrimaryKeyConstraint(
                        'import_id', name=op.f('pk__imports'))
                    )
    op.create_table('citizens',
                    ...
                    sa.ForeignKeyConstraint(['import_id'], ['imports.import_id'], name=op.f(
                        'fk__citizens__import_id__imports')),
                    sa.PrimaryKeyConstraint('import_id', 'citizen_id',
                                            name=op.f('pk__citizens'))
                    )
    op.create_index(op.f('ix__citizens__town'),
                    'citizens', ['town'], unique=False)
    op.create_table('relations',
                    sa.Column('import_id', sa.Integer(), nullable=False),
                    sa.Column('citizen_id', sa.Integer(), nullable=False),
                    sa.Column('relative_id', sa.Integer(), nullable=False),
                    sa.ForeignKeyConstraint(['import_id', 'citizen_id'], ['citizens.import_id', 'citizens.citizen_id'], name=op.f(
                        'fk__relations__import_id_citizen_id__citizens')),
                    sa.ForeignKeyConstraint(['import_id', 'relative_id'], ['citizens.import_id', 'citizens.citizen_id'], name=op.f(
                        'fk__relations__import_id_relative_id__citizens')),
                    sa.PrimaryKeyConstraint('import_id', 'citizen_id',
                                            'relative_id', name=op.f('pk__relations'))
                    )
